...
def _create_tiny_network(num_classes=14, dropout=GLOBAL_DROPOUT, filter_inc=6, starting_filters=32, kern_size=16):
...
def _create_medium_network(num_classes=14, dropout=GLOBAL_DROPOUT, filter_inc=4, starting_filters=64, kern_size=16):
...
def _create_large_network(num_classes=14, dropout=GLOBAL_DROPOUT, filter_inc=4, starting_filters=64, kern_size=16):
...
def _create_residual_network(resi_layer,num_classes=14, dropout=GLOBAL_DROPOUT, filter_inc=4, starting_filters=64, kern_size=16):
    ...
    shortcut = prev_layer
    prev_layer = Conv1D(starting_filters, kern_size, padding='same', kernel_regularizer=keras.regularizers.l1(0.01))(prev_layer)
    prev_layer = BatchNormalization()(prev_layer)
    prev_layer = ReLU()(prev_layer)
    ...
    for x in range(1,resi_layer+1):
        ...
        prev_layer = ReLU()(prev_layer)
        prev_layer = Dropout(dropout)(prev_layer)
        prev_layer = Conv1D(starting_filters*(2**factor), kern_size, padding='same', kernel_regularizer=keras.regularizers.l1(0.01))(prev_layer)
        ...
        shortcut = Conv1D(starting_filters*(2**factor), 1, padding='same', kernel_regularizer=keras.regularizers.l1(0.01))(shortcut)
        shortcut = MaxPooling1D(pool_size=1)(shortcut)
    ...
    prev_layer = Dense(num_classes, kernel_regularizer=keras.regularizers.l1(0.01))(prev_layer)
...
def train_model(model, ecg_data, ecg_labs, lr=0.001, decay=0.0, epochs=1, validation_data=(None, None), class_weight=None, batch_size=1024, callbacks=None):
    model.compile(loss='categorical_crossentropy',
          optimizer=keras.optimizers.Adam(lr=lr, beta_1=0.9, beta_2=0.999, epsilon=None, decay=decay, amsgrad=False),
          metrics=['accuracy'])
    model.fit(ecg_data, ecg_labs, epochs=epochs, validation_data=validation_data, class_weight=class_weight, batch_size=batch_size, callbacks=callbacks)
